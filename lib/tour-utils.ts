import { TourConfig, TourState, AnalyticsEvent } from '@/types/tour';
...
export const tourStorage = {
  ...
  saveState: (tourId: string, state: TourState): void => {
    try {
      localStorage.setItem(`${STORAGE_KEY_PREFIX}${tourId}`, JSON.stringify(state));
    } catch (e) {
  ...
  markTourCompleted: (tourId: string): void => {
    try {
      ...
      if (!completed.includes(tourId)) {
        ...
        localStorage.setItem(`${STORAGE_KEY_PREFIX}completed`, JSON.stringify(completed));
      }
    } catch (e) {
